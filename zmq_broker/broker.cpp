...
void receive_from_ue1(std::vector<std::complex<float>> &buffer_vec,void* req_socket_from_ue_1_tx, int nbytes, int &size){
...
void receive_from_ue2(std::vector<std::complex<float>> &buffer_vec_ue_2, void* req_socket_from_ue_2_tx, int nbytes, int &size_ue_2, int tx_data_count, int tx_count_lim, float pl_ue_2){
...
std::pair<string, string> extract_port(string device_args){
...
int main(int argc, char *argv[]){
    ...
    vector<pair<string,string>> ports_collection; //all ports for ue {tx,rx}
    ...
    for(int config_num = 1; config_num < argc; ++config_num){
        ...
        if (find(ports_collection.begin(), ports_collection.end(), ports) != ports_collection.end()) {
            cout << "\nThis ue already exist\n";
            cout << "\n===== End parsing " << config_file << " =====\n";
            continue;
        }

        ports_collection.push_back(ports);
    ...
    std::cout << "\nZMQ_MAX_BUFFER_SIZE = " << ZMQ_MAX_BUFFER_SIZE << std::endl;
    ...
    std::vector<std::complex<float>> buffer_vec_ue_2(N);
    std::vector<std::complex<float>> buffer_vec_to_gnb(N);
    ...
    while(1){
        ...
        if (summ >= 1  && broker_rcv_accept_gnbs[0] == num_gnbs)
        {
            ...
            std::thread thread_ue1([&]() {
                receive_from_ue1(std::ref(buffer_vec), req_socket_from_ue_1_tx, nbytes, std::ref(size));
            });

            std::fill(buffer_vec_to_gnb.begin(), buffer_vec_to_gnb.end(), 0);
    
            std::thread thread_ue2([&]() {
                receive_from_ue2(std::ref(buffer_vec_ue_2), req_socket_from_ue_2_tx, nbytes, 
                                std::ref(size_ue_2), tx_data_count, tx_count_lim, pl_ue_2);
            });
