 ...
 */
 ...
 scheduler_time_lwdf::scheduler_time_lwdf(const scheduler_ue_expert_config& expert_cfg_, du_cell_index_t cell_index_) :
   params(std::get<time_lwdf_scheduler_expert_config>(expert_cfg_.strategy_cfg)), cell_index(cell_index_)
 {
 }
 
 void scheduler_time_lwdf::add_ue(du_ue_index_t ue_index)
 {
 ...
 void scheduler_time_lwdf::rem_ue(du_ue_index_t ue_index)
 {
   ue_history_db.erase(ue_index);
 }
 
 void scheduler_time_lwdf::compute_ue_dl_priorities(slot_point               pdcch_slot,
 ...
 void scheduler_time_lwdf::compute_ue_ul_priorities(slot_point               pdcch_slot,
 ...
 void scheduler_time_lwdf::save_dl_newtx_grants(span<const dl_msg_alloc> dl_grants)
 {
 ...
 void scheduler_time_lwdf::save_ul_newtx_grants(span<const ul_sched_info> ul_grants)
 {
 ...
 double combine_qos_metrics(double                                  pf_weight,
                            ...
                            double                                  prio_weight,
                            double                                  delay_weight,
                            const time_lwdf_scheduler_expert_config& policy_params)
 {
   if (policy_params.qos_weight_func == time_lwdf_scheduler_expert_config::weight_function::gbr_prioritized and
       gbr_weight > 1.0) {
 ...
 scheduler_time_lwdf::ue_ctxt::ue_ctxt(du_ue_index_t             ue_index_,
                                      du_cell_index_t           cell_index_,
                                      const scheduler_time_lwdf* parent_) :
   ue_index(ue_index_),
 ...
 void scheduler_time_lwdf::ue_ctxt::compute_dl_prio(const slice_ue& u,
                                                   ...
                                                   unsigned        nof_slots_elapsed)
 {
   ...
   if (parent->params.gbr_enabled or parent->params.priority_enabled or parent->params.pdb_enabled) {
    for (logical_channel_config_ptr lc : *u.logical_channels()) {
      if (not u.contains(lc->lcid) or not lc->qos.has_value() or u.pending_dl_newtx_bytes(lc->lcid) == 0) {
        // LC is not part of the slice, No QoS config was provided for this LC or there is no pending data for this LC
        continue;
      ...
      if (hol_toa.valid() and pdcch_slot >= hol_toa) {
        ...
        const unsigned pdb          = lc->qos->qos.packet_delay_budget_ms;
        per = lc->qos->qos.per.to_double();
        qos_weight = -std::log2(per) / static_cast<double>(pdb);
        delay_weight += hol_delay_ms / static_cast<double>(pdb);
      }
    }
  }
  
  dl_prio = qos_weight * (delay_weight)* pf_weight;
 
 }
 
 void scheduler_time_lwdf::ue_ctxt::compute_ul_prio(const slice_ue& u,
                                                   slot_point      pdcch_slot,
                                                   slot_point      pusch_slot,
                                                   unsigned        nof_slots_elapsed)
 {
  ...
  Алгоритму требуется задержка пакета (hol_delay_ms) и бюджет задержки (pdb), а также qos_weight, который вычисляется по формуле:
 ...
 void scheduler_time_lwdf::ue_ctxt::compute_dl_avg_rate(const slice_ue& u, unsigned nof_slots_elapsed)
 {
 ...
 void scheduler_time_lwdf::ue_ctxt::compute_ul_avg_rate(const slice_ue& u, unsigned nof_slots_elapsed)
 {
   // In case more than one slot elapsed.
   if (nof_slots_elapsed > 1) {
 ...
 void scheduler_time_lwdf::ue_ctxt::save_dl_alloc(uint32_t total_alloc_bytes, const dl_msg_tb_info& tb_info)
 {
   dl_sum_alloc_bytes += total_alloc_bytes;
 }
 
 void scheduler_time_lwdf::ue_ctxt::save_ul_alloc(unsigned alloc_bytes)
 {
   if (alloc_bytes == 0) {
     return;
   }
