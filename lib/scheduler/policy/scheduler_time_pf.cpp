...
scheduler_time_pf::scheduler_time_pf(const scheduler_ue_expert_config& expert_cfg_, du_cell_index_t cell_index_) :
  params(std::get<time_pf_scheduler_expert_config>(expert_cfg_.strategy_cfg)), cell_index(cell_index_)
{
}
...
double combine_qos_metrics(double                                  pf_weight,
                           ...
                           double                                  prio_weight,
                           double                                  delay_weight,
                           const time_pf_scheduler_expert_config& policy_params)
{
  if (policy_params.qos_weight_func == time_pf_scheduler_expert_config::weight_function::gbr_prioritized and
      gbr_weight > 1.0) {
...
scheduler_time_pf::ue_ctxt::ue_ctxt(du_ue_index_t             ue_index_,
                                     du_cell_index_t           cell_index_,
                                     const scheduler_time_pf* parent_) :
  ue_index(ue_index_),
